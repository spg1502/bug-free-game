...
class DebugOverlay:
    ...
    def __init__(self, batch):
        self.batch = batch
        self.insert_debug_line(debuggable.Debuggable("Debug Overlay", "(press 0 to toggle)"))

    def update_debug(self):
        for idx, line in enumerate(self.debuggable_contents):
            if idx >= len(self.debug_lines):
                self.debug_lines.append(utils.create_debug_label(line.get_debug_output(), y_start = self.y_pos, batch = self.batch))
                self.y_pos -= 14
            else:
